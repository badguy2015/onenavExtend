...
//事件执行
    var active = {
        ...
        importcategory:function(){import_category();},
        exportCategoryAndLink:function(){exportCategoryAndLink();}
    };
//链接搜索
    function link_q(){
    ...
    table.on('toolbar(mylink)', function(obj){
        ...
        switch(obj.event){
            ...
            case 'importlink':
                ...
                import_link();
                break;
            case 'importlinkOverwrite':
                // 导入导航
                import_link_overwrite();
                break;
            case 'exportCategoryAndLink':
                // 导出导航
                exportCategoryAndLink();
                break;
            case 'zhiding':
                var data = checkStatus.data;
                if( data.length == 0 ) {
...
//导入分类目录
    function import_category(){
        ...
        if(!(config && config.category)) return false;
        $.post('./index.php?c=api&method=import_category&u='+u, config,function(data,status){
            //如果添加成功
            if(data.code == 0) {
                // layer.msg('已添加！', {icon: 1});
                layer.msg(data.msg);
...
//导入 导航书签
    function import_link(){
        let config = {};
        config = JSON.parse(localStorage.getItem('config'));
        if(!(config && config.navigation)) return false;
        $.post('./index.php?c=api&method=import_link&u='+u, config,function(data,status){
            //如果添加成功
            if(data.code == 0) {
                // layer.msg('已添加！', {icon: 1});
                layer.msg(data.msg);
...
//导入 导航书签(覆盖)
    function import_link_overwrite(){
        ...
        config = JSON.parse(localStorage.getItem('config'));
        if(!(config && config.navigation)) return false;
        $.post('./index.php?c=api&method=import_link_overwrite&u='+u, config,function(data,status){
            //如果添加成功
            if(data.code == 0) {
                // layer.msg('已添加！', {icon: 1});
                layer.msg(data.msg);
...
// 导出 导出书签到localStorage
    function exportCategoryAndLink(){
        $.post('./index.php?c=api&method=export_category_and_link&u='+u, {},function(data,status){
            //如果添加成功
            console.log(data);
            if(data.code == 0) {
...
//识别链接信息
    ...
    window.bookmarks_ = function(){
        window.open('./index.php?c=api&method='+ 'export_html' +'&u='+u +'&pass=' + $.md5(value));
    }
//书签管理
